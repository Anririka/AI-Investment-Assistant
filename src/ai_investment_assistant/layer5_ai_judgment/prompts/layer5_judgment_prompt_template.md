...
6. 全候補（採用・不採用問わず）について、採否・理由・除外理由コードを含む判断ログ
   （`decision_log`相当の情報）を生成する。**各エントリには`ticker`に加えて`name`
   （銘柄名）を必ず含めること**（2026-08-12追加。Layer6の「除外・不採用ログ」シートに
   証券コードだけでなく銘柄名も表示してほしいというユーザー要望対応。`name`はLayer2
   出力の`candidates`（採用検討した銘柄）・`excluded_summary`（母集団フィルタ等で
   事前に除外された銘柄）のいずれにも含まれているので、該当する`ticker`のエントリから
   そのまま転記すればよく、新たに調べたり創作したりする必要はない）。

   なお、`enforce_daily_limit`（6-3手順4）が機械的に生成する`not_selected`エントリは
   `name`を含めて自動生成されるため、あなたが手を加える必要はない。あなたが直接
   組み立てる必要があるのは、あなた自身の判断で`decision: "rejected"`（LLMが不採用と
   判断した候補）とする分と、Layer2の`excluded_summary`をそのまま転記する分のみである。
