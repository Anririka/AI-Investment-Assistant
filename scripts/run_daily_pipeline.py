...
値は暫定のフォールバックとして扱い、上位の人間レビューに委ねる）：
  ...
  1. 【市場レジーム判定用の指数ティッカー（2026-07-24対応、解消）】regime_detector.
     detect_regime()は指数PriceSeriesを要求するが、当初は日経平均株価（プレースホルダー
     コード"998407"、Yahoo!ファイナンス上の表記）を個別銘柄用の`get_daily_prices`で
     取得しようとしており、常に失敗していた（2026-07-23〜24のライブ実行で確認）。
     調査の結果、日経平均株価はそもそも日本経済新聞社の別途ライセンスが必要でJ-Quantsでは
     提供されておらず、コードを直しても解決しないことが判明した。J-Quants自身が公式に
     提供する指数であるTOPIX（東証株価指数、指数コード"0000"）に切り替え、専用の
     `get_index_daily_prices`メソッド（`/indices/bars/daily`エンドポイント、
     jquants.py参照）を使うようにした。取得失敗時は引き続き「レンジ相場」に
  ...
  5. 【market_capが日本株・米国株とも常時取得不能だった問題（2026-07-23対応、解消）】
     J-Quants（無料〜Lightプラン）の`/equities/master`、Alpha Vantage/Twelve Dataの
     `get_listed_universe()`はいずれも時価総額を直接は返さない（2026-07-23のライブ実行で
     確認、全候補がMARKET_CAP_TOO_SMALLで除外されていた）。日本株はまず本スクリプト側
     （`_estimate_market_cap`）の近似計算（純利益÷EPS×直近終値）で対応し、ライブ実行で
     実際に候補が出ることを確認した。米国株はこの近似が使えなかった（Alpha Vantage
     OVERVIEWがnet_incomeを提供しないため）ことが判明したため、ユーザーの承認を得て
     FundamentalSnapshotに`market_cap`フィールドを追加した（models.py、デフォルト値
     Noneの末尾フィールドとして後方互換を保っている）。Alpha Vantage
     （`MarketCapitalization`）・Twelve Data（`market_capitalization`）はいずれも
     この新フィールドへ直接値を設定する。

     時価総額の優先順位は`_fetch_market_candidates`内で以下のとおり（`_estimate_market_cap`
     のdocstringも参照）：
         1. info.market_cap（get_listed_universeが直接返す値、現状どの情報源も未提供）
         2. fundamentals.market_cap（情報源が直接提供する値、Alpha Vantage/Twelve Data）
         3. _estimate_market_cap（純利益÷EPS×直近終値の近似値、J-Quants向けの最終手段）
     3番目のみ近似値であり、発行済株式数の変動（自己株買い等）や、EPSが古い期のもので
     ある場合には実際の時価総額とズレが生じ得る点は変わらず注意が必要。
"""
...
# ギャップ1（2026-07-24解消）：市場レジーム判定の基準指数。日経平均株価はJ-Quantsで
# 提供されていないため、J-Quants自身が公式に提供するTOPIX（東証株価指数）へ切り替えた。
# "0000"はTOPIX自体の指数コード（J-Quants `/indices/bars/daily`のindexcodes一覧より）。
MARKET_REGIME_INDEX_CODE = "0000"  # TOPIX
...
def _fetch_market_candidates(
    ...
    for ticker in tickers:
        try:
            ...
            # 時価総額の優先順位（2026-07-23追加、A案）：
            #   1. info.market_cap（get_listed_universeが直接返す値、現状どの情報源も未提供）
            #   2. fundamentals.market_cap（Alpha Vantage OVERVIEWの`MarketCapitalization`・
            #      Twelve Data `/statistics`の`market_capitalization`等、情報源が直接提供する値）
            #   3. _estimate_market_cap（純利益÷EPS×直近終値の近似値、J-Quantsのように
            #      直接値もfundamentals.market_capも無い場合の最終フォールバック）
...
def _fetch_index_series(japan_chain, start: date, end: date, critical_errors: list) -> PriceSeries:
    try:
        return japan_chain.call("get_index_daily_prices", MARKET_REGIME_INDEX_CODE, start, end)
    except DataSourceError as exc:
        logger.error("index series fetch failed, falling back to range regime: %s", exc)
        critical_errors.append(
            {
                "code": "PRICE_DATA_INVALID",
                "message": f"市場レジーム判定用の指数（TOPIX、コード{MARKET_REGIME_INDEX_CODE}）取得に失敗: {exc}",
                "source_layer": "layer1",
            }
        )
        return _empty_price_series(MARKET_REGIME_INDEX_CODE, str(exc))
...
def main() -> int:
    ...
    if japan_chain is not None:
        ...
        index_price_series = _empty_price_series(MARKET_REGIME_INDEX_CODE, "japan_equity chain unavailable")
