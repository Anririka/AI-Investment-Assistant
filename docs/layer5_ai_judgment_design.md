...
日本株・米国株・ETF等いずれも: 推奨株数 = floor(推奨株数(仮))    # 整数株に切り下げ

損切価格 = 購入価格 × (1 - 0.10)   # 基本方針。LLMが個別に変更を提言した場合のみ別値（要理由）
利確価格 = 購入価格 × (1 + take_profit_target_pct / 100)   # 確定：目標騰落率方式を基本とする
```

**2026-07-26変更（デザインフリーズ後の確定仕様変更、要記録）**：日本株の推奨株数は当初
「東証の単元株制度（100株単位）」を前提にfloor(仮/100)×100としていたが、テスト期間中の
縮小運用資金（25万円）に対し、対象ユニバースの日本株がいずれも100株購入の最低必要額
（例：トヨタ自動車終値2,897円→289,700円）が総資金自体を上回り、構造的に一切約定
できない問題が実データで確認された。ユーザーの実際の証券口座（SBI証券）が単元未満株
サービス「S株」（1株単位の売買）に対応していることを確認したうえで、ユーザーの明示
判断により、日本株も米国株と同様に1株単位（floor(仮)）へ変更した
（`position_sizer.py`の`_floor_shares`参照）。単元未満株は成行注文のみ・リアルタイム
約定ではなく所定時刻（前場9:00・12:30、後場15:30、夜間21:00等、証券会社により異なる）
の一括約定である点は、実際の発注執行運用上の考慮事項として別途認識しておくこと
（`position_sizer.py`の計算自体には影響しない）。
...
| `position_sizer.py` | 元の計算式（33%上限・残余資金・整数株単位切り下げ。2026-07-26より日本株も1株単位、§8参照）の既知の入力値に対する期待値照合。複数候補の逐次配分で合計が300万円を超えないこと。`take_profit_target_pct`から利確価格が正しく算出されること |
